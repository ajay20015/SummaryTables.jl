    ...
    for row in 1:size(matrix, 1)        
        ...
        for col in 1:size(matrix, 2)
            ...
            if index == 0
                ...
                push!(rowcells, docx_cell(row, col, cell, rowgaps, colgaps, measures, ct.merge_row_labels))
                running_index = index
...
function docx_cell(row, col, cell, rowgaps, colgaps, measures, merge_row_labels)
